...
export function StepAmenities({
    ...
    return (
        <div className="space-y-6 max-w-3xl mx-auto w-full">
            <h2 className="text-2xl font-bold text-white mb-4">What does it have?</h2>
                    ...
                    <div className="flex flex-wrap gap-2 mt-4">
                        {formData.customAmenities.map((item, i) => (
                            <span key={i} className="flex items-center gap-2 px-3 py-1.5 bg-blue-500/20 text-blue-200 text-sm rounded-lg border border-blue-500/30">
                                {item}
                                <button onClick={() => removeArrayItem('customAmenities', i)} className="hover:text-white"><X className="h-3 w-3" /></button>
                            </span>
                        ))}
                    </div>
                )}
                    ...
                    <div className="flex flex-col gap-2 mt-4">
                        {formData.rules.map((rule, i) => (
                            <div key={i} className="flex items-center justify-between px-4 py-2 bg-red-500/10 text-red-200 text-sm rounded-xl border border-red-500/20">
                                <span>{i + 1}. {rule}</span>
                                <button onClick={() => removeArrayItem('rules', i)} className="hover:text-white p-1"><X className="h-4 w-4" /></button>
                            </div>
                        ))}
                    </div>
                )}
