...
// Host Photo Component for Organizer Display
const HostPhoto = React.memo(({ host }) => {
    ...
    const hostPhoto = useMemo(() => {
        ...
        // Check all possible photo field names from multer-s3
        const photoFields = [
            'selfie_photo',      // Most likely field name from multer-s3
            'selfiePhoto',        // CamelCase version
            'profile_photo',      // Alternative field name
            'profilePhoto',       // CamelCase version
            'avatar',             // Generic avatar field
            'photo',              // Generic photo field
            'image',              // Generic image field
            'profileImage'        // CamelCase version
        ];
...
// Memoized EventCard component with all required fields
const EventCard = React.memo(({ event, viewMode, onViewDetails, index }) => {
    ...
    const getEventImage = () => {
        ...
        return "https://images.unsplash.com/photo-1492684223066-81342ee5ff30"; // Default fallback
    };
...
// Memoized HorizontalEventCard component with all required fields
...
const EventsPage = () => {
  ...
  const eventsByCategory = useMemo(() => {
    if (!apiEvents || apiEvents.length === 0) return MOCK_EVENTS; // Fallback to mocks if no API data yet
    ...
    apiEvents.forEach(event => {
      // Normalize category or use default
      ...
      if (grouped[categoryId]) {
        ...
        // Handle unknown categories if needed, or add to 'other'
          ...
          return (
            ...
            >
              ...
              <div className="relative">
                ...
                <div className="absolute inset-y-0 right-0 w-16 sm:w-24 from-white to-transparent pointer-events-none" />
              </div>
            </section>
          )
        })}
